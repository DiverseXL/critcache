...
const banner = `
${chalk.green(`  [✦] critcache`)} ${chalk.dim(`v${version}`)}
${chalk.dim(`  parallel AI code review · powered by BTL Runtime`)}
${chalk.dim(`  ${'─'.repeat(49)}`)}
`;
...
program
  .name("critcache")
  .description(
    "Fan out parallel AI code-review agents across a repo through BTL Runtime, " +
...
function resolveFiles(repo: string, maxFiles: number) {
  let files: ReturnType<typeof walkRepo>;
  try {
    files = walkRepo(repo, { maxFiles });
  } catch (err) {
    console.error(chalk.red(`\nFailed to walk repo: ${(err as Error).message}\n`));
    process.exitCode = 1;
    return null;
  }
...
program
  ...
  .action(
    async (
      repo: string,
      opts: { maxFiles: string; concurrency: string; output: string },
    ) => {
      console.log(banner);
      ...
      console.log(
        chalk.dim(
          `Found ${files.length} file(s) to review` +
      ...
      table.push(
        ...
        ["Benchmark cost (no caching)", `$${result.totalChargeUsd.toFixed(4)}`],
        ["Actual charge (via BTL)",     `$${result.totalBenchmarkCostUsd.toFixed(4)}`],
        [
      ...
      if (result.totalChargeUsd > 0) {
        const pct = (result.totalSavedUsd / result.totalChargeUsd) * 100;
...
program
  .command("compare")
  .description(
    "Run analysis twice on the same repo back-to-back to prove BTL Runtime's " +
  ...
  .action(
    async (
      repo: string,
      opts: { maxFiles: string; concurrency: string },
    ) => {
      console.log(banner);
      ...
      console.log(
        chalk.dim(
          "Running the same analysis twice, back to back, " +
      ...
      const hitRate1 =
        comparison.pass1.rows.length > 0
          ? (comparison.pass1.cacheHits / comparison.pass1.rows.length) * 100
          : 0;
      const hitRate2 =
        comparison.pass2.rows.length > 0
