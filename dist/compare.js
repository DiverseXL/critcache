 ...
 */
export async function runComparison(files, concurrency) {
...
export function renderComparisonTable(result) {
    ...
    const hitRate1 = pass1.rows.length > 0 ? (pass1.cacheHits / pass1.rows.length) * 100 : 0;
    const hitRate2 = pass2.rows.length > 0 ? (pass2.cacheHits / pass2.rows.length) * 100 : 0;
    table.push([
        "Cache hit rate",
        `${hitRate1.toFixed(0)}%`,
